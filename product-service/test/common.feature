...
Feature: Common utilities and authentication
  ...
  @ignore
  Scenario: Verify product service is up and running
    * def productServiceInstUrl = replace(karate.get('productServiceUrl'),'product','product-inst')
    * url productServiceInstUrl
    Given path 'healthz'

-- Commands to create a user [manual execution] by root user
CREATE USER optisam_app_user WITH PASSWORD '*****';
GRANT CONNECT ON DATABASE product TO optisam_app_user;
GRANT CONNECT ON DATABASE dps TO optisam_app_user;
GRANT CONNECT ON DATABASE account TO optisam_app_user;
GRANT CONNECT ON DATABASE application TO optisam_app_user;
GRANT CONNECT ON DATABASE report TO optisam_app_user;
GRANT CONNECT ON DATABASE simulation TO optisam_app_user;
GRANT USAGE ON SCHEMA public TO optisam_app_user;

--Below commands to be executed by optisam user
// Account Database 
ALTER TABLE groups ADD is_group_compliance BOOLEAN DEFAULT FALSE;

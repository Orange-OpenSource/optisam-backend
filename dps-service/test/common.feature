...
Feature: Common utilities and authentication
  ...
  Background: pre-requisite
    ...
    * def todayDate = function(format){ return new java.text.SimpleDateFormat(format).format(new java.util.Date())}
    * def sort = 
      """
  ...
  @ignore
  Scenario: Verify dps service is up and running
    * def dpsServiceInstUrl = replace(karate.get('dpsServiceUrl'),'dps','dps-inst')
    * url dpsServiceInstUrl
    Given path 'healthz'

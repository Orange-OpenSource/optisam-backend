...
Feature: E2E Data injection test for GEN Scope

## Pre-requisite :  
# 1. Nifi flow must be running
# 2. Equipment type should be present in GEN scope
# 3. dps cron timing should be less then 4,5 minutes
 ...
 Scenario: Verify end to end data upload for GENERIC scope using Global Data files
    ## Delete Inventory
    ...
    * header Accept = '*/*'
    ...
    # * def file1_tmp = karate.readAsString('importv026.xlsx')
    * multipart file file1 = { read: 'importv026.xlsx', filename: 'importv026.xlsx', contentType: 'multipart/form-data'}
    ...
    * def upload_id = response.uploads[0].upload_id
    * call pause 40000
    Given path 'uploads/data'
    * header Authorization = 'Bearer '+access_token
    * params { page_num:1, page_size:10, sort_by:'upload_id', sort_order:'desc', scope:'#(scope)', global_file_id:'#(upload_id)'}
    When method get
    Then status 200 
    * match response.uploads[0].uploaded_on contains today
    # * match response.uploads[1].uploaded_on contains today

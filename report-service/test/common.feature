...
Feature: Common utilities and authentication
  ...
  @ignore
  Scenario: Verify report service is up and running
    * def reportServiceInstUrl = replace(karate.get('reportServiceUrl'),'report','report-inst')
    * url reportServiceInstUrl
    Given path 'healthz'

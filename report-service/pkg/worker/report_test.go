...
import (
	...
	dmock "gitlab.tech.orange/optisam/optisam-it/optisam-services/report-service/pkg/repository/v1/dmock"
	...
	ls "gitlab.tech.orange/optisam/optisam-it/optisam-services/report-service/thirdparty/license-service/pkg/api/v1"
	mockls "gitlab.tech.orange/optisam/optisam-it/optisam-services/report-service/thirdparty/license-service/pkg/api/v1/mock"
	"testing"
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				dmockRepo.EXPECT().ProductEquipments(ctx, "e1", "Scope1", "partition").Return([]*repv1.ProductEquipment{
					{
						Swidtag: "p1",
						Equipments: []*repv1.Equipment{
							{
								EquipmentID:   "e11",
				...
				gomock.InOrder(
					...
					dmockRepo.EXPECT().EquipmentParents(ctx, "e11", "partition", "Scope1").Return([]*repv1.Equipment{
						{
							EquipmentID:   "pp1",
					...
					dmockRepo.EXPECT().EquipmentParents(ctx, "e2", "partition", "Scope1").Return([]*repv1.Equipment{
						{
							EquipmentID:   "pp3",
							EquipmentType: "server",
						},
		...
		},
		{
			...
			args: args{
				...
				j: &job.Job{
					Type:   sql.NullString{String: "rw"},
					Status: job.JobStatusPENDING,
					Data:   []byte(`{"report_type":"AcqRightsReport","scope":"Scope1","json":{"editor":"e1"},"report_id":1}`),
				},
			},
			setup: func() {
				mockCtrl = gomock.NewController(t)
				...
				dmockRepo := dmock.NewMockDgraphReport(mockCtrl)
				drep = dmockRepo
				mocklicenseClient.EXPECT().GetOverAllCompliance(ctx, &ls.GetOverAllComplianceRequest{Editor: "e1", Scope: "Scope1"}).Times(1).Return(&ls.GetOverAllComplianceResponse{
				...
				finaljson := []byte(`[{"sku":"sku1","aggregationName":"","swidtags":"p1","editor":"e1","metric":"metric1","computedLicenses":1000,"computationDetails":"","acquiredLicenses":10000,"delta(number)":9000,"delta(cost)":100,"totalcost":104.5,"avgunitprice":2.5},{"sku":"sku2","aggregationName":"","swidtags":"p2","editor":"e1","metric":"metric2","computedLicenses":1001,"computationDetails":"","acquiredLicenses":10001,"delta(number)":9001,"delta(cost)":100.01,"totalcost":104.6,"avgunitprice":2.6}]`)
				mockrepo.EXPECT().InsertReportData(ctx, db.InsertReportDataParams{
					ReportID:       int32(1),
					ReportDataJson: finaljson,
				}).Return(nil).Times(1)
		...
		},
		{
			...
			setup: func() {
				...
				finaljson := []byte(`[{"sku":"sku1","aggregationName":"","swidtags":"p1","editor":"e1","metric":"metric1","computedLicenses":1000,"computationDetails":"","acquiredLicenses":10000,"delta(number)":9000,"delta(cost)":100,"totalcost":104.5,"avgunitprice":2.5}]`)
				mockrepo.EXPECT().InsertReportData(ctx, db.InsertReportDataParams{
					ReportID:       int32(1),
					ReportDataJson: finaljson,
				}).Return(nil).Times(1)
		...
		},
		{
			...
			setup: func() {
				...
				mockrepo.EXPECT().UpdateReportStatus(ctx, db.UpdateReportStatusParams{
					ReportStatus: db.ReportStatusFAILED,
					ReportID:     int32(1),
				}).Times(1).Return(nil)
			},
			wantErr: false,
		...
		},
		{
			...
			setup: func() {
				...
				mocklicenseClient.EXPECT().GetOverAllCompliance(ctx, &ls.GetOverAllComplianceRequest{Editor: "e1", Scope: "Scope1"}).Times(1).Return(&ls.GetOverAllComplianceResponse{}, errors.New("service error"))
				mockrepo.EXPECT().UpdateReportStatus(ctx, db.UpdateReportStatusParams{
					ReportStatus: db.ReportStatusFAILED,
					ReportID:     int32(1),
				}).Times(1).Return(nil)

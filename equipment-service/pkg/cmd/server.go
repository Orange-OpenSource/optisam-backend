...
import (
	...
	"optisam-backend/equipment-service/pkg/config"
	"optisam-backend/equipment-service/pkg/protocol/grpc"
	"optisam-backend/equipment-service/pkg/protocol/rest"
	repo "optisam-backend/equipment-service/pkg/repository/v1/dgraph"
	v1 "optisam-backend/equipment-service/pkg/service/v1"
	"os"
	"time"
...
func RunServer() error {
	...
	if bserr := os.Setenv("DEL_BATCH_SIZE", cfg.DelBatchSize); bserr != nil {
		logger.Log.Info("del batch size env variable not set, error: %v", zap.Error(bserr))
	}
	...
	rep := repo.NewEquipmentRepository(dg)

	v1API := v1.NewEquipmentServiceServer(rep, grpcClientMap)

...
Feature: Common utilities and authentication
  ...
  @ignore
  Scenario: Verify simulation service is up and running
    * def simulationServiceInstUrl = replace(karate.get('simulationServiceUrl'),'simulation','simulation-inst')
    * url simulationServiceInstUrl
    Given path 'healthz'

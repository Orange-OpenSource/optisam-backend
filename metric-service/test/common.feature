...
Feature: Common utilities and authentication
  ...
  @ignore
  Scenario: Verify metric service is up and running
    * def metricServiceInstUrl = replace(karate.get('metricServiceUrl'),'metric','metric-inst')
    * url metricServiceInstUrl
    Given path 'healthz'

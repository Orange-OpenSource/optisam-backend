...
Feature: To validate CRUD operation on metrics : admin user
  ...
  Scenario Outline: To verify user can create all metrics
   ...
   # * def ibm_payload = {"Name":"ibm_pvu","num_core_attr_id":'#(eq_data.serv_core.ID)',"core_factor_attr_id":'#(eq_data.server_oracle.ID)',"numCPU_attr_id":'#(eq_data.serv_processor.ID)',"base_eq_type_id":'#(eq_data.server_eq_type.ID)', "scopes":['#(scope)']}
     * def ibm_payload = {"Name":"ibm_pvu","num_core_attr_id":'#(eq_data.serv_core.ID)',"numCPU_attr_id":'#(eq_data.serv_processor.ID)',"core_factor_attr_id":'#(eq_data.server_oracle.ID)',"base_eq_type_id":'#(eq_data.server_eq_type.ID)', "scopes":['#(scope)']}
  ...
  Scenario Outline: To verify Delete created Metric
    Given path <path>
    ...
    | 'metric/instance_8' |
    | 'metric/static_1' |
    | 'metric/attribute_sum' |
    | 'metric/hyperthreading' |
    | 'metric/user' |

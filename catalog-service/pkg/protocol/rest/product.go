...
SELECT id, name, editorid, genearl_information, contract_tips, support_vendors, metrics, licences_opensource, location, created_on, updated_on, recommendation, useful_links,swid_tag_product,opensource_type,editor_name,licensing,(Select json_agg(t.scope) as a from (
...
func (handler *handler) GetProduct(w http.ResponseWriter, r *http.Request) {
	...
	var scopes []byte
	var support_vendors, metrics, usefulLinks []byte
	...
	if scopes != nil {
		...
		scps, err := handler.pCRepo.GetScope(r.Context(), scopeCode)
		if err != nil {
			logger.Log.Error("couldn't fetch scopes from redis", zap.Any("error", err))
		}
		if len(scps) == len(scopeCode) {
			...
			responseObject.Scopes = scopeCode
		} else {
...
const innerScopeProducts = `
INNER JOIN (
	SELECT * FROM (
	SELECT product_editor,product_name FROM products WHERE scope = Any($27)
	UNION
	SELECT product_editor,product_name FROM acqrights WHERE scope = Any($27)
	GROUP BY product_editor,product_name
	) as parkData
	) AS pp ON pp.product_editor= pc.editor_name AND pp.product_name = pc.name
`
const listProducts = `
...
SELECT count(*) OVER() AS totalRecords,pc.id, pc.name, pc.editorid, pc.genearl_information, pc.contract_tips, pc.support_vendors, pc.metrics, pc.licences_opensource, pc.location, pc.created_on, pc.updated_on, pc.recommendation, pc.useful_links ,pc.swid_tag_product,pc.editor_name,pc.opensource_type,pc.licensing,json_agg(v.* order by v.name asc) as ve
from product_catalog pc
left outer join version_catalog v on v.p_id=pc.id and v.name !=''`
...
WHERE
 (CASE WHEN $1::BOOL THEN lower(pc.name) LIKE '%' || lower($2::TEXT) || '%' ELSE TRUE END)
 AND
 (CASE WHEN $3::BOOL THEN pc.location::TEXT = Any($4) ELSE TRUE END)
 AND
 (CASE WHEN $5::BOOL THEN pc.recommendation::TEXT = Any($6) ELSE TRUE END)
 AND
 (CASE WHEN $7::BOOL THEN pc.licensing::TEXT = Any($8) ELSE TRUE END)
 AND
 (CASE WHEN $9::BOOL THEN support_vendors ?| $10 ELSE TRUE END)
AND
 (CASE WHEN $23::BOOL THEN lower(editor_name) LIKE '%' || lower($24::TEXT) || '%' ELSE TRUE END)
 AND
(CASE WHEN $25::BOOL THEN editorID = $26::TEXT ELSE TRUE END)
GROUP BY pc.id 
ORDER BY
CASE WHEN $11::BOOL THEN created_on END asc,
CASE WHEN $12::BOOL THEN created_on END desc,
CASE WHEN $13::BOOL THEN pc.name END asc,
CASE WHEN $14::BOOL THEN pc.name END desc,
CASE WHEN $15::BOOL THEN location END asc,
CASE WHEN $16::BOOL THEN location END desc,
CASE WHEN $17::BOOL THEN editor_name END asc,
CASE WHEN $18::BOOL THEN editor_name END desc,
CASE WHEN $21::BOOL THEN licensing END asc,
CASE WHEN $22::BOOL THEN licensing END desc
OFFSET $19 LIMIT $20 ) as product
`
...
func (handler *handler) GetProducts(w http.ResponseWriter, r *http.Request) {
	...
	err := validateProdPagination(r)
	if err != nil {
		logger.Log.Error("rest - geteditor - pagination error", zap.String("Reason: ", err.Error()))
		sendErrorResponse(http.StatusMethodNotAllowed, err.Error(), w)
		return
	}
	...
	dep := r.URL.Query().Get("search_params.deploymentType.filteringkey")
	deploymentType := strings.Split(dep, ",")
	isdepType := (dep != "")

	vendor := r.URL.Query().Get("search_params.vendor.filteringkey")
	VendorType := strings.Split(vendor, ",")
	isVendorType := (vendor != "")

	recommendation := r.URL.Query().Get("search_params.recommendation.filteringkey")
	recommendationType := strings.Split(recommendation, ",")
	isrecommendationType := (recommendation != "")

	licensing := r.URL.Query().Get("search_params.licensing.filteringkey")
	licensingType := strings.Split(licensing, ",")
	isLicensingType := (licensing != "")
	...
	if isEntitiesType {
		...
		query = listProducts + innerScopeProducts + whereCondition
		rows, err = handler.Db.QueryContext(r.Context(), query, ispname, pname, isdepType, pq.Array(deploymentType),
			isrecommendationType, pq.Array(recommendationType),
			isLicensingType, pq.Array(licensingType),
			isVendorType, pq.Array(VendorType),
			CreatedOnAsc, CreatedOnDesc, NameAsc,
			NameDesc, locationTypeAsc, locationTypeDesc, eNameAsc,
			eNameDesc, PageNum, PageSize, licensingasc, licensingdesc, iseditorname, pditorname, iseditorId, pditorId, pq.Array(scopeCodes))
	} else {
		query = listProducts + whereCondition
		rows, err = handler.Db.QueryContext(r.Context(), query, ispname, pname, isdepType, pq.Array(deploymentType),
			isrecommendationType, pq.Array(recommendationType),
			isLicensingType, pq.Array(licensingType),
			isVendorType, pq.Array(VendorType),
			CreatedOnAsc, CreatedOnDesc, NameAsc,
			NameDesc, locationTypeAsc, locationTypeDesc, eNameAsc,
			eNameDesc, PageNum, PageSize, licensingasc, licensingdesc, iseditorname, pditorname, iseditorId, pditorId)
	}
	...
	for rows.Next() {
		...
		var support_vendors, metrics, usefulLinks, versions, scopes []byte
		var swidTagProduct, openLicences sql.NullString
		if err := rows.Scan(
	...
	if len(productsDbResponse) > 0 {
		...
		for v := range productsDbResponse {
			...
			apiresp.Product[v].Licensing = string(productsDbResponse[v].Licensing)
...
func validateProdPagination(r *http.Request) error {
	...
	pn, err := strconv.Atoi(r.URL.Query().Get("page_num"))
	if err != nil {
		return err
	}
	if pn < 1 || pn > 1000 {

...
func (l *LicenseRepository) ProductAcquiredRights(ctx context.Context, swidTag string, metrics []*v1.Metric, isSimulation bool, scopes ...string) (string, string, []*v1.ProductAcquiredRight, error) {
	...
	return data.Products[0].ID, data.Products[0].ProductName, prodRights, nil
}
	...
	q := `
		{
			Products(func: uid(` + pID + `)) {
				...
				ProductEquipment: product.equipment @filter(eq(equipment.type,"server") AND eq(scopes,[` + scopes[0] + `]) AND not(has(equipment.parent))){
					EUID : 	uid
					EquipmentId  :	equipment.id
				}
			}
...
func (l *LicenseRepository) ConcatAcqRightForSameMetric(ctx context.Context, metrics []*v1.Metric, acqRight []*productAcquiredRight, isSimulation bool, scope string) []*v1.ProductAcquiredRight {
	...
	if ok {
		...
		metricNup, _ := l.GetMetricConfigNUPID(ctx, nupMetric, scope)
		if metricNup.Transform {
			opsMetric, ok := metricType[v1.MetricOPSOracleProcessorStandard]
			if ok && metricNup.TransformMetricName == opsMetric {

...
func TestLicenseRepository_Equipments(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("LicenseRepository.Equipments() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("LicenseRepository.Equipments() got = %v, want %v", got, tt.want)
			}
...
func TestLicenseRepository_Equipment(t *testing.T) {
	eqTypes, cleanup, err := equipmentSetup(t)
	if !assert.Empty(t, err, "error not expected as cleanup") {
		return
	}
...
func TestLicenseRepository_EquipmentParent(t *testing.T) {
	eqTypes, cleanup, err := equipmentSetup(t)
	if !assert.Empty(t, err, "error not expected as cleanup") {
		return
	}
...
func TestLicenseRepository_EquipmentChild(t *testing.T) {
	eqTypes, cleanup, err := equipmentSetup(t)
	if !assert.Empty(t, err, "error not expected as cleanup") {
		return
	}
...
func TestLicenseRepository_EquipmentProducts(t *testing.T) {
	eqTypes, cleanup, err := equipmentSetup(t)
	if !assert.Empty(t, err, "error not expected as cleanup") {
		return
	}
	...
	uidNewScope, err := getUIDForEquipmentXIDWithType("equip44_1", "MyType2")
	if !assert.Empty(t, err, "error not expected from getUIDForEquipmentXIDWithType") {
		return
	}
	uid1, err := getUIDForEquipmentXIDWithType("equip4_7", "MyType2")
	...
	type args struct {
		...
		eqType *v1.EquipmentType
		id     string
		params *v1.QueryEquipmentProduct
		scopes []string
	}
	tests := []struct {
		name         string
		...
		args         args
		want         int32
		wantProducts []*v1.EquipmentProduct
		wantErr      bool
	}{
		{name: "success , sort by swidtag all filters",
			r: NewLicenseRepository(dgClient),
			args: args{
				ctx:    context.Background(),
				eqType: eqType,
				...
				params: &v1.QueryEquipmentProduct{
					PageSize:  5,
					Offset:    0,
					SortBy:    v1.EquipmentProductSortBySwidTag,
					SortOrder: v1.SortASC,
					Filter: &v1.AggregateFilter{
						Filters: []v1.Queryable{
							&v1.Filter{
								FilterKey:   v1.EquipmentProductSearchKeySwidTag.String(),
								FilterValue: "ORAC00",
							},
							&v1.Filter{
								FilterKey:   v1.EquipmentProductSearchKeyName.String(),
								FilterValue: "ORACLE SGBD",
							},
							&v1.Filter{
								FilterKey:   v1.EquipmentProductSearchKeyEditor.String(),
								FilterValue: "oracle",
							},
							&v1.Filter{
								FilterKey:   v1.EquipmentProductSearchKeyVersion.String(),
								FilterValue: "9.2.0",
							},
							&v1.Filter{
								FilterKey:   v1.EquipmentProductSearchKey("unknown").String(),
								FilterValue: "9.2.0",
							},
						},
					},
		...
		{name: "success , sort by swidtag desc offset 1",
			r: NewLicenseRepository(dgClient),
			args: args{
				ctx:    context.Background(),
				eqType: eqType,
		...
		{name: "success , sortby name",
			...
			args: args{
				...
				params: &v1.QueryEquipmentProduct{
					PageSize:  5,
					Offset:    0,
					SortBy:    v1.EquipmentProductSortByName,
					SortOrder: v1.SortDESC,
				},
				scopes: []string{"scope1", "scope2"},
			},
		...
		{name: "success , sort by version asc",
			r: NewLicenseRepository(dgClient),
			args: args{
				ctx:    context.Background(),
				eqType: eqType,
				...
				params: &v1.QueryEquipmentProduct{
					PageSize:  5,
					Offset:    0,
					SortBy:    v1.EquipmentProductSortByVersion,
					SortOrder: v1.SortDESC,
					Filter: &v1.AggregateFilter{
						Filters: []v1.Queryable{
							&v1.Filter{
		...
		{name: "success , sort by editor asc",
			r: NewLicenseRepository(dgClient),
			args: args{
				ctx:    context.Background(),
				eqType: eqType,
				...
				params: &v1.QueryEquipmentProduct{
					PageSize:  5,
					Offset:    0,
					SortBy:    v1.EquipmentProductSortByEditor,
					SortOrder: v1.SortDESC,
					Filter: &v1.AggregateFilter{
						Filters: []v1.Queryable{
							&v1.Filter{
		...
		{name: "success , sort by swidtag desc offset 1 - scope 3",
			r: NewLicenseRepository(dgClient),
			args: args{
				ctx:    context.Background(),
				eqType: eqType,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			numOfRecords, got, err := tt.r.EquipmentProducts(tt.args.ctx, tt.args.eqType, tt.args.id, tt.args.params, tt.args.scopes)
			if (err != nil) != tt.wantErr {
				t.Errorf("LicenseRepository.Equipments() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			...
			if !tt.wantErr {
				if !assert.Equal(t, tt.want, numOfRecords, "num of records should be equal") {
					return
				}
				compareEquipmentProductAll(t, "EquipmentsProducts", tt.wantProducts, got)
			}
		})
	}
}

...
func Test_licenseServiceServer_wssMetricCalulation(t *testing.T) {
	ctx := grpc_middleware.AddClaims(context.Background(), &claims.Claims{
		UserID: "admin@superuser.com",
		Role:   "Admin",
		Socpes: []string{"scope1", "scope2"},
	})
	metrics := []*repo.MetricWSS{
		{
			MetricName: "metric1",
	...
	tests := []struct {
		...
		{name: "SUCCESS - individual",
			...
			setup: func() {
				...
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricWSS(ctx, gomock.Any()).AnyTimes().Return(metrics, nil)
				mockRepo.EXPECT().MetricWSSComputedLicensesAgg(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(uint64(1), nil)
			},
		},
		{name: "SUCCESS - individual err",
			args: args{
				ctx: ctx,
				req: map[string]interface{}{
					"scopes":        []string{"scope1", "scope2"},
			...
			setup: func() {
				...
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricWSS(ctx, gomock.Any()).AnyTimes().Return(metrics, nil)
				mockRepo.EXPECT().MetricWSSComputedLicensesAgg(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(uint64(1), sql.ErrNoRows)
			},
			wantErr: true,
		},
		...
		{name: "SUCCESS - individual iss agg false",
			...
			setup: func() {
				...
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricWSS(ctx, gomock.Any()).AnyTimes().Return(metrics, nil)
				mockRepo.EXPECT().MetricWSSComputedLicenses(ctx, gomock.Any(), gomock.Any()).AnyTimes().Return(uint64(1), nil)
			},
		},
		{name: "SUCCESS - err",
			args: args{
			...
			setup: func() {
				...
				mockProdClient := mockpro.NewMockProductServiceClient(mockCtrl)
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricWSS(ctx, gomock.Any()).AnyTimes().Return(metrics, sql.ErrNoRows)
			},
			wantErr: true,
		},
		{name: "SUCCESS - not found",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := wssMetricCalulation(tt.args.ctx, s, tt.args.eqTypes, tt.args.req)

...
func Test_licenseServiceServer_mssMetricCalulation(t *testing.T) {
	ctx := grpc_middleware.AddClaims(context.Background(), &claims.Claims{
		UserID: "admin@superuser.com",
		Role:   "Admin",
		Socpes: []string{"scope1", "scope2"},
	})
	metrics := []*repo.MetricMSS{
		{
			MetricName: "metric1",
	...
	tests := []struct {
		...
		{name: "SUCCESS - individual",
			...
			setup: func() {
				...
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricMSS(ctx, gomock.Any()).AnyTimes().Return(metrics, nil)
				mockRepo.EXPECT().MetricMSSComputedLicensesAgg(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(uint64(1), nil)
			},
		},
		...
		{name: "SUCCESS - individual iss agg false",
			...
			setup: func() {
				...
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricMSS(ctx, gomock.Any()).AnyTimes().Return(metrics, nil)
				mockRepo.EXPECT().MetricMSSComputedLicenses(ctx, gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(uint64(1), nil)
			},
		},
		{name: "SUCCESS - err",
			args: args{
			...
			setup: func() {
				...
				mockProdClient := mockpro.NewMockProductServiceClient(mockCtrl)
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricMSS(ctx, gomock.Any()).AnyTimes().Return(metrics, sql.ErrNoRows)
			},
			wantErr: true,
		},
		{name: "SUCCESS - not found",
			...
			setup: func() {
				...
				prod = mockProdClient
				mockRepo.EXPECT().ListMetricMSS(ctx, gomock.Any()).AnyTimes().Return(metrics, nil)
				mockRepo.EXPECT().MetricMSSComputedLicensesAgg(ctx, gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(uint64(1), sql.ErrNoRows)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := mssMetricCalulation(tt.args.ctx, s, tt.args.eqTypes, tt.args.req)
